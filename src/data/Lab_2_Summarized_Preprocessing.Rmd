---
title: "Lab 2 Summarized Preprocessing"
...
Load the person data into the variable "data"
...
Create a new column for number of people dead per crash

```{r}

# Group by ST_CASE and count the number of fatalities
data <- data %>%
  group_by(ST_CASE) %>%
  mutate(DEATH = sum(INJ_SEV == 4)) %>%
  ungroup()

# If you want to convert DEATH column to factor
data$DEATH <- as.factor(data$DEATH)

```

Create a column for the total number of people involved in each crash

```{r}

# Calculate the number of people involved for each ST_CASE
data <- data %>%
  group_by(ST_CASE) %>%
  mutate(PPL_INVOLVED = n())

```

Create a column for the number of injured or dead people in each crash

```{r}

# Create a new column to represent the count of values 2, 3, or 4 in INJ_SEV for each ST_CASE
data <- data %>%
  group_by(ST_CASE) %>%
  mutate(NUMBER_OF_INJ_or_DEAD = sum(INJ_SEV %in% c(2, 3, 4))) %>%
  ungroup()

```

Create an indicator if a driver in the crash was under the influence of alcohol.
...
I want an indicator to say if the driver was under the influence of drugs
...
Calculate the percentage of people involved in the crash that died
...
Calculate the percentage of people involved in the crash that got injured or died.
...
Calculate the number of seat belts used by people involved in the crash.
...
Calculate the percentage of airbags that deployed in the crash.
...
Calculate the number of airbags that deployed in the crash.
...
We want to calculate the average age in each crash.
...
Create a new data.frame with summarized information on each crash
...
Plot the average age of drivers in the crash against the percentages of injury or deaths, highlighting if Alcohol was involved in Red.
...
Create a column that tells us how many drivers were in each crash
